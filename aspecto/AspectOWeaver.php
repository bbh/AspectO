...
class AspectOWeaver {
  ...
  public function __construct( AspectOConfig &$config, Aspect &$aspect )
  {
    ...
    if ( isset( $aspects ) ) {
      ...
      foreach ( $aspects as $class_name => $_aspect ) {
        ...
        file_put_contents( $config->getWeavedClassesPath() . $class_name.'.php',
                           $weave->getString() );
        }
      }
    }
   ...
   * @return AspectOClassParser
   */
  final protected static function WeaveMethods ( AspectOClassParser &$class,
                                                 $methods )
  {
    if ( is_array( $methods ) ) {
   ...
   * @return Array
   */
  final protected static function getMethodsToWeave ( Aspect $aspect,
                                                      AspectOClassParser $class )
  {
    if ( is_array( $aspect->pointcuts ) ) {
      ...
      foreach ( $aspect->pointcuts as $pointcut ) {
        ...
        if ( is_array( $pointcut->joinpoint->method ) ) {
          ...
          $_methods = self::getMethodFromClass( $class, $pointcut->joinpoint->method->name );
      ...
      if ( isset( $aspect->pointcuts->joinpoint->method ) &&
           !is_null( $aspect->pointcuts->joinpoint->method ) ) {
        ...
        if ( is_array( $aspect->pointcuts->joinpoint->method ) ) {
          ...
          $_methods = self::getMethodFromClass( $class, $aspect->pointcuts->joinpoint->method->name );
   ...
   * @return Array
   */
  final protected static function getArrayOfMethodToWeave ( $methods, $pointcut,
                                                            $advices )
  {
    if ( $advice_from_pointcut = self::getAdviceFromPointcut( $advices,
                                                              $pointcut->name ) ) {
   ...
   * @return Method
   */
  final protected static function getMethodFromClass ( AspectOClassParser $class,
                                                       $method_name )
  {
   ...
   * @return Mixed
   */
  final protected static function getObjectByClass ( $aspect_node, $class,
                                                     $element )
  {
